...
FlutterVulkanImage FlutterGetNextImageCallback(
  ...
  // Assumes that there will not be swap chains of length greater than 4.
  int age = 4;
...
bool FlutterPresentCallback(void* user_data, const FlutterVulkanImage* image) {
  ...
  damage_history_.push_back(image->frame_damage.damage[0]);
  if (damage_history_.size() > kMaxHistorySize) {
    damage_history_.pop_front();
  }

  // TODO: User should use the image damage that was passed to render only that
  // specific area of the screen at present time.
